...
![image-20220401193010260](Cpp.assets/image-20220401193010260.png)
...
如果要使用引用可在线程传参中使用`std::ref(a)`

```cpp
template<class T>
void print_add(const T &a, const T &b, T c)
{
...
int main()
{
    ...
    int &c = a;
    std::cout << &a << std::endl;
    std::thread obj(print_add<int>, std::ref(a), b, c);
    obj.join();
}
```

得到变量`a`和`print_add()`参数`a,b,c`的地址

```
0xb3387ff944
0xb3387ff944 0x2aa4aeaee6c 0xb338fff9e0
```

2.

```cpp
void print_add(const T &a)
```

如果线程从主线程`detach`了，在子线程的函数入口处，使用引用来传递参数，但是该引用不是真正的引用，不再是地址传递，实际上为值传递，重新拷贝一个值并重新申请一块内存给了子线程

即使主线程运行完毕了，子线程用`a`仍然是安全的，但仍不推荐传递引用，推荐改为`const int a`

```cpp
void print_add(T *a)
```

使用指针来传递参数，该指针仍然指向的主线程中申请的地址

`a`指向原来的字符串，当主线程中的的局部变量被释放，子线程对应的内存也就不安全了

```cpp
void print_add(const std::string &a) { }


char str[] = "Hello World";
std::thread obj(print, str);
obj.detach();
```

`str`由`char`类型数组转到`std::string`的对象需要一次拷贝构造，需要一定时间，期间如果str离开作用域就可能使构造失败，所以尽量提前拷贝构造

```cpp
std::thread obj(print, std::string(str));
```
...
public:
    ...
    A(int) { std::cout << "int constructed" << std::endl; }
    A(const A &) { std::cout << "copy constructed" << std::endl; }
    ~A() { std::cout << "destructed" << std::endl; }
};

void print(const A &str)
{
    //为了区分析构 作为分界线
...
int main
{
    std::thread obj_1(print, A(1));	
    //用1构造了对象A(1)，A(1)拷贝给了函数参数str
    
    //std::std::thread obj_1(print, 1);
    //Sleep(1000);	//如果没有可能不会有输出
    //直接用1构造A类型参数
